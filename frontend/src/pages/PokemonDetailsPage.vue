<template>
  <section class="shell">
    ...
    <header class="nav">
      ...
      <div class="nav-tools">
        <span class="chip" v-if="data"
          >#{{ String(data.number).padStart(3, '0') }} • {{ data.name }}</span
        >
    ...
    <div v-else-if="data" class="content">
      ...
      <aside class="stack">
        ...
        <article class="card">
          <h3>Typy</h3>
          <div class="types">
            <span v-for="t in data.types" :key="t" class="type">
              {{ t }}
            </span>
          </div>
        </article>
        ...
        <article class="card">
          <h3>Zdolności</h3>
          <div class="types">
            <span v-for="ab in data.abilities" :key="ab" class="type">
              {{ ab }}
            </span>
          </div>
        </article>
        ...
        <article class="card" aria-labelledby="stats">
          ...
          <div class="stats">
            <div class="row" v-for="(val, key) in orderedStats" :key="key">
              ...
              <div
                ...
                <div
                  class="fill"
                  :style="{
                    width: Math.min(100, Math.round((val / 255) * 100)) + '%',
                  }"></div>
              </div>
        ...
        <article class="card">
          ...
          <div class="moves">
            <ul class="moves-grid">
              <li v-for="m in filteredMoves" :key="m" class="move">
                {{ m }}
              </li>
            </ul>
          </div>
...
:root {
  ...
  --font:
    ui-sans-serif, -apple-system, blinkmacsystemfont, 'Segoe UI', 'SF Pro Text',
...
.dot {
  ...
  box-shadow: 0 0 16px rgb(245 196 0 / 45%);
}
...
.muted-weak {
  color: color-mix(in oklab, var(--muted) 70%, #fff 0%);
}
...
.type {
  ...
  padding: 6px 10px;
  font-weight: 600;
  background: linear-gradient(180deg, rgb(245 196 0 / 15%), transparent);
  font-size: 12px;
}
...
.bar {
  height: 10px;
  border-radius: 999px;
  background: color-mix(in oklab, var(--ring) 90%, transparent);
  overflow: hidden;
  position: relative;
}
