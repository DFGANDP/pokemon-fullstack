        ...
        for p in data:
            ...
            for t in p.get("types", []):
                ...
                if key not in seen_types:
                    session.add(PokemonType(
                        pokemon_number=p["number"],
                        pokemon_type=t_lower
                    ))
                    seen_types.add(key)
            ...
            for ability in p.get("abilities", []):
                ...
                if key not in seen_abilities:
                    session.add(PokemonAbility(
                        pokemon_number=p["number"],
                        pokemon_ability=ability_lower
                    ))
                    seen_abilities.add(key)

            evo = p.get("evolution", {})
            from_name = evo.get("from")
            for to_name in evo.get("to") or []:
                if from_name and to_name:
                    session.add(PokemonEvolution(
